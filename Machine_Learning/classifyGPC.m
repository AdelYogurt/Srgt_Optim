...
legend('','高精度正样本点','高精度负样本点','高精度分类边界','Location','southoutside','Orientation','horizontal','NumColumns',3)
axis equal;
axes_handle.set('Position',[0.1300,0.2500,0.7750,0.6750]);
figure_handle.Children(1).set('Position',[0.1300,0.100,0.7750,0.050])
figure_handle.set('Position',[488   200   560   560])
...
%     end
% 
%     function [Y_pred,possibility,miu_pred,var_pred] = classifyGaussPredictor...
%             (X_pred,hyp,mean,cov,lik,post,X,aver_X,stdD_X)
%         % predict function
%         %
%         X_pred_nomlz = (X_pred-aver_X)./stdD_X;
...
%         nperbatch = 1000;                       % number of data points per mini batch
%         nact = 0;                       % number of already processed test data points
%         ymu = zeros(ns,1); ys2 = ymu; miu_pred = ymu; var_pred = ymu; possibility = ymu;   % allocate mem
%         while nact<ns               % process minibatches of test cases to save memory
%             id = (nact+1):min(nact+nperbatch,ns);               % data points to process
%             kss = feval(cov{:},hyp.cov,X_pred_nomlz(id,:),'diag');              % self-variance
%             Ks = feval(cov{:},hyp.cov,X(nz,:),X_pred_nomlz(id,:));        % avoid computation
...
%             N = size(alpha,2);  % number of alphas (usually 1; more in case of sampling)
%             Fmu = repmat(ms,1,N) + Ks'*full(alpha(nz,:));        % conditional mean fs|f
%             miu_pred(id) = sum(Fmu,2)/N;                                   % predictive means
%             if Lchol    % L contains chol decomp => use Cholesky parameters (alpha,sW,L)
%                 V  = L'\(repmat(sW,1,length(id)).*Ks);
%                 var_pred(id) = kss - sum(V.*V,1)';                       % predictive variances
%             else                % L is not triangular => use alternative parametrisation
%                 if isnumeric(L),LKs = L*Ks; else LKs = L(Ks); end    % matrix or callback
%                 var_pred(id) = kss + sum(Ks.*LKs,1)';                    % predictive variances
%             end
%             var_pred(id) = max(var_pred(id),0);   % remove numerical noise i.e. negative variances
%             Fs2 = repmat(var_pred(id),1,N);     % we have multiple values in case of sampling
%             if nargin<9
%                 [Lp,Ymu,Ys2] = feval(lik{:},hyp.lik,[],Fmu(:),Fs2(:));
%             else
%                 Ys = repmat(ys(id),1,N);
...
%         n = length(y);                                      % number of training cases
%         sW = sqrt(ttau);                                        % compute Sigma and mu
%         matrix=eye(n)+sW*sW'.*K;
%         L = chol(matrix);                            % L'*L = B = eye(n)+sW*K*sW
%         V = L'\(repmat(sW,1,n).*K);
%         Sigma = K - V'*V;
%         alpha = tnu-sW.*(L\(L'\(sW.*(K*tnu+m))));
%         mu = K*alpha+m; v = diag(Sigma);
