function srgtVisualize(axe_hdl,model,low_bou,up_bou,...
...
pred_fcn=@(x) model.predict(x);

% get boundary
if (nargin < 2 || isempty(low_bou)), low_bou=min(x_list,[],1);end
if (nargin < 3 || isempty(up_bou)), up_bou=max(x_list,[],1);end

if numel(low_bou) ~= numel(low_bou)
    error('surrogateVisualize: boundary incorrect');
end
...
switch dim
    case 1
        ...
        fval_pred_mat=zeros(grid_num+1,1);
        for x_index=1:grid_num+1
            x_pred=(x_index-1).*d_bou+low_bou;
            fval_pred_mat(x_index)=pred_fcn(x_pred);
        end

        fval_pred_mat=min(fval_pred_mat,Y_max);
        fval_pred_mat=max(fval_pred_mat,Y_min);
        y_list=min(y_list,Y_max);
        y_list=max(y_list,Y_min);

        line(axe_hdl,x_mat,fval_pred_mat);
    ...
    case 2
        ...
        try
            ...
            x_pred=[x_mat(:),y_mat(:)];
            fval_pred_mat=pred_fcn(x_pred);
            fval_pred_mat=reshape(fval_pred_mat,grid_num+1,grid_num+1);
        catch
            % if not support multi input
            fval_pred_mat=zeros(grid_num+1);
            for x_index=1:grid_num+1
                for y_index=1:grid_num+1
                    x_pred=([x_index,y_index]-1).*d_bou+low_bou;
                    [fval_pred_mat(y_index,x_index)]=pred_fcn(x_pred);
                end
            end
        end
        ...
        surface(axe_hdl,x_mat,y_mat,fval_pred_mat,'EdgeColor','none','FaceAlpha',0.5)
    ...
    otherwise
        ...
        try
            ...
            for x_idx=1:grid_num+1
                for y_idx=1:grid_num+1
                    ...
                    fval_pred_mat(y_idx,x_idx)=pred_fcn(predict_x);
                end
            end
        end
        ...
        surface(x_mat,y_mat,fval_pred_mat,'FaceAlpha',0.5,'EdgeColor','none');
