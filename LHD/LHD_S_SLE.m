...
if ~isempty(X_exist)
    ...
    place = repmat(0:(variable_number-1),exist_number,1);
    place = place(:)*total_number;
    feasible_list(remove_index_list+place) = [];
    
    if (length(feasible_list)/variable_number) ~= ...
            (total_number-size(X_exist_nomlz,1))
        save('matlab.mat');
...
grid_list = zeros(total_number,variable_number);

if ~isempty(X_exist)
   grid_list(1:size(X_exist,1),:) = grid_exist_list; 
end

for sample_new_index = exist_number+1:total_number
    % each sampling will decrease one gird of each variable
    grid_available_number = total_number-sample_new_index+1;
...
% remapping method
X_sample_nomlz = (grid_list(exist_number+1:end,:)-0.5)/(total_number);
