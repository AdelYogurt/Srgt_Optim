classdef OptimKRGEGO < handle
    ...
    methods
        function self=OptimKRGEGO(NFE_max,iter_max,obj_torl,con_torl)
            ...
            model_option.('simplify_hyp')=false;
            model_option.('hyp')=[];
            ...
            option_optim.KRG_option=model_option;
            option_optim.adapt_hyp=false;

            % strategy parameter option
            option_optim.scaling_factor=0.8; % F
            option_optim.cross_rate=0.8;
        ...
        function [result_X,result_Obj,result_Con,result_Coneq,result_Vio]=sampleAdapt...
            ...
            self.Srgt_obj=repmat({model_option},1,obj_num);
            self.Srgt_con=repmat({model_option},1,con_num);
            self.Srgt_coneq=repmat({model_option},1,coneq_num);
            ...
            while ~done
                ...
                % add infill point
                [x_infill,infill_fcn]=self.searchInfill(vari_num,low_bou,up_bou,self.option_optim.criteria,obj_min);
    ...
    % strategy function
    methods
        function [x_infill,infill_fcn]=searchInfill(self,vari_num,low_bou,up_bou,criteria,obj_min)
            % MSP, EI, PI, MSE, LCB infill criteria
            %
            switch criteria
                case 'MSP'
                    infill_fcn=@(x) self.obj_fcn_srgt(x);
                    con_fcn=@(x) self.con_fcn_srgt(x);
                case 'EI'
                    w_l=0.5;
                    infill_fcn=@(x) -EIFcn(x,self.obj_fcn_srgt,self.con_fcn_srgt,obj_min,w_l);
                    con_fcn=[];
                case 'PI'
                    infill_fcn=@(x) -PIFcn(x,self.obj_fcn_srgt,self.con_fcn_srgt,obj_min);
                    con_fcn=[];
                case 'MSE'
                    infill_fcn=@(x) -MSEFcn(x,self.obj_fcn_srgt,self.con_fcn_srgt);
                    con_fcn=[];
                case 'LCB'
                    infill_fcn=@(x) LCBFcn(x,self.obj_fcn_srgt,self.con_fcn_srgt);
                    con_fcn=@(x) self.con_fcn_srgt(x);
            end

            x_init=rand(1,vari_num).*(up_bou-low_bou)+low_bou;
            ...
            [x_infill,~]=run(gs,problem);

            function EI=EIFcn(x,obj_fcn_srgt,con_fcn_srgt,obj_min_surr,w_l)
                w_g=1-w_l;
                ...
                if isempty(con_fcn_srgt),return;end
                EI=EI*calFeasProb(con_fcn_srgt,x);
            end

            function PI=PIFcn(x,obj_fcn_srgt,con_fcn_srgt,obj_min_surr)
                [obj_pred,obj_var]=obj_fcn_srgt(x);
                obj_nomlz=(obj_min_surr-obj_pred)/sqrt(obj_var);
                PI=normcdf(obj_nomlz);

                if isempty(con_fcn_srgt),return;end
                PI=PI*calFeasProb(con_fcn_srgt,x);
            end

            function MSE=MSEFcn(x,obj_fcn_srgt,con_fcn_srgt)
                [~,obj_var]=obj_fcn_srgt(x);
                MSE=obj_var;

                if isempty(con_fcn_srgt),return;end
                MSE=MSE*calFeasProb(con_fcn_srgt,x);
            end

            function LCB=LCBFcn(x,obj_fcn_srgt,con_fcn_srgt)
                [obj_pred,obj_var]=obj_fcn_srgt(x);
                LCB=obj_pred-sqrt(obj_var);

                if isempty(con_fcn_srgt),return;end
                LCB=LCB*calFeasProb(con_fcn_srgt,x);
            end

            function feas_prob=calFeasProb(con_fcn_srgt,x)
                % calculate feasible probability
                %
                feas_prob=1;
                [con_pred,coneq_pred,con_var,coneq_var]=con_fcn_srgt(x);
                if ~isempty(con_pred)
                    if any(con_pred > 0)
                        feas_prob=0;
                        return;
                    end
                    con_feas_prob=normcdf(-con_pred./sqrt(con_var));
                    feas_prob=feas_prob*prod(con_feas_prob);
                end
            end
        end
    end
