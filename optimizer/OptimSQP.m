...
[x_best,fval_best,NFE,output]=...
    optimSQP(object_function,x_initial,A,B,Aeq,Beq);
...
FLAG_INFORMATION=1; % whether show information
...
x_init=x_init(:);
vari_num=size(x_init,1);

if isempty(iter_max)
iter_max=10*vari_num;
end
if isempty(NFE_max)
    NFE_max=100*vari_num;
end
...
if HESS_UPDATA_FLAG==1
    hessian_fcn=@(x_delta,gradient_delta,H) hessianDFPMethod(x_delta,gradient_delta,H);
elseif HESS_UPDATA_FLAG==2
    hessian_fcn=@(x_delta,gradient_delta,H) hessianBFGSMethod(x_delta,gradient_delta,H);
end

NFE=0;done=0;iteration=0;
result_x_best=zeros(iter_max,vari_num);
result_fval_best=zeros(iter_max,1);
...
while ~done
    ...
    [x_new,fval_new,gradient_new,lamada_best,NFC_line]=lineSearch...
        (obj_fcn,direction,x_init,fval_initial,gradient_initial,lamada,...
        Aineq,Bineq,Aeq,Beq,low_bou,up_bou,con_fcn,...
    ...
    %     end
    H=hessian_fcn(x_delta,gradient_delta,H);
